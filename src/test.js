function main({ pane, glslVersion, contextID }) {
    ...
    function initParticlesArrays() {
        ...
        const numParticles = Math.round(
            width * height * PARAMS.particleDensity
        );
        const positions = new Float32Array(
            numParticles * PARTICLES_NUM_COMPONENTS
        );
        const heading = new Float32Array(numParticles);
        for (let i = 0; i < numParticles; i++) {
            positions[PARTICLES_NUM_COMPONENTS * i] = Math.random() * width;
            positions[PARTICLES_NUM_COMPONENTS * i + 1] =
                Math.random() * height;
    ...
    function getParticlesFolderTitle() {
        return `Particles (${particlesPositions.length.toLocaleString(
            "en-US"
        )})`;
    }
    const ui = [];
    ...
    particlesGUI
        ...
        .on("change", (e) => {
            updateParticles.setUniform(
                "u_sensorAngle",
                (PARAMS.sensorAngle * Math.PI) / 180
            );
        });
    particlesGUI
        .addInput(PARAMS, "sensorDistance", {
            min: 1,
            max: 30,
            step: 0.01,
    ...
    particlesGUI
        ...
        .on("change", (e) => {
            updateParticles.setUniform(
                "u_rotationAngle",
                (PARAMS.rotationAngle * Math.PI) / 180
            );
        });
    particlesGUI
        .addInput(PARAMS, "stepSize", {
            min: 0.01,
            max: 3,
            step: 0.01,
            label: "Step Size",
    ...
    particlesGUI
        .addInput(PARAMS, "particleDensity", {
            min: 0.01,
            max: 1,
            step: 0.01,
    ...
    renderGUI
        .addInput(PARAMS, "renderAmplitude", {
            min: 0,
            max: 1,
            step: 0.01,
            label: "Amplitude",
